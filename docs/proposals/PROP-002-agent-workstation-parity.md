...
- **IS-1 擴充鍵列**：終端 UI 上方常駐鍵列，至少含 Esc / Ctrl（組合鍵模式）/ Tab / Shift+Tab / 方向鍵；支援長按重複。對應工作流依賴：coding agent TUI 的中斷（Esc）與模式切換（Shift+Tab）。**可達性要求（Phase 1 實測補）**：核心鍵不可藏在需水平捲動才看得到的位置——現成 client（Termius）的 Esc 存在但被捲動的鍵列推到畫面外遮住、一度被誤判為缺鍵，是實際會用不下去的形態。設計上核心鍵預設可見、捲動區只放次要鍵；並可提供終端通用等價鍵作保險（`Ctrl+[` 送出與 Esc 相同的 `0x1b`，任何終端等價）。
...
- **IS-4 全螢幕 TUI 相容性**：coding agent 的全螢幕介面在 app 內渲染正確——色彩、游標定位、畫面更新頻率、scrollback 行為以 Phase 1 的現成 client 表現為 baseline 對照。渲染須正確處理 CJK 雙寬字元（一個中文字佔兩欄）、否則游標定位與行重繪會錯位。
- **IS-5 CJK 即時輸入（Phase 1 揭露的差異化機會）**：終端輸入要能即時打中文、而不只是貼上。現成 client（Termius）的終端 raw 模式擋掉輸入法（IME）的即時組字流程、只能他處組好再貼進來——這是它最實在的行動端輸入缺口。自製 client 在這裡有結構性優勢：Flutter 端可設計一個 compose buffer / 輸入列，讓 IME 先完成「組字 → 選候選字」再把成品送進終端，繞過 raw 模式的限制。對應工作流依賴：用中文跟 coding agent 對話（純英文 / ASCII 靠貼上頂著、但中文對話是實際需求）。這一項是 app_tunnel 相對現成 client 最值得做的差異化——不是追平、是超越。

### 本提案不做的（Out of Scope）

- **mosh 式本地回顯預測** → tailnet 內 RTT 低（同城直連），預測回顯的工程量對體感增益不成比例；接受為與現成 client（mosh 路線）的 known gap，對照測時記錄體感差距，差距顯著再開獨立提案。附帶好處：app_tunnel 走 WebSocket、不做本地預測，先天避開了「mosh 預測撞 CJK 雙寬字導致中文輸入行顯示錯位」這個問題——現成 client 的 mosh 路線開中文輸入時會錯位、只能退回純 SSH（失去漫遊）。這是 app_tunnel 在 CJK 場景相對 mosh 路線的先天優勢，對照測時應凸顯（與 IS-5 相輔）。
...
- [ ] **AC-4（對應 IS-4）**：coding agent 全螢幕 TUI 的渲染與 Phase 1 baseline 對照無阻礙性差異（色彩 / 游標 / 更新 / scrollback 逐項記錄）；CJK 雙寬字顯示無錯位。
- [ ] **AC-5（對應 IS-5）**：手機端能**即時輸入**中文（非貼上）跟 agent 對話、組字流程順、送出正確——這是超越 Phase 1 現成 client（只能貼上）的驗收點。
- [ ] **AC-6（連線失敗 UX）**：連不上時 app 的錯誤訊息能區分「逾時（多半是未加入 tailnet）」與「被拒（服務層問題）」、引導使用者往正確方向排查（Phase 1 踩過「手機不在 tailnet → 逾時、卻直覺去查伺服器」的誤判）。
...
| 類別 | 內容 |
|------|------|
| 已驗證 | **假設 1 獲 Phase 1 正面佐證**：現成 client 實測 zellij session 在斷線（含 mosh 漫遊與純 SSH 斷線兩種）期間任務未中斷、重連 attach 接回、輸出連續無斷檔——印證 client 端只需「重連後回到同一 session」即可、session 復原由 remote multiplexer 承擔。本提案的 IS-2 風險因此降一級。 |
| 未驗證 | 假設 2（WebSocket 在網路切換的重連體感）、假設 3（Flutter 終端渲染的 TUI 相容性、含 CJK 雙寬字）仍待本提案實作時驗 |
...
### 2026-07-08（Phase 1 現成 client 實測回饋）

外部工作流 Phase 1 以 Termius（Android）實機跑通十步驟 + 三情境，過程揭露幾項具體開發注意事項、回饋進本提案的 In Scope 與 AC：

- **擴充鍵列「可達性」比「有沒有」更關鍵**（回饋進 IS-1）：Termius 的 Esc 存在、但被水平捲動的鍵列推到畫面外遮住、一度誤判為缺鍵。核心鍵必須預設可見、不可藏在捲動區；並記終端通用等價鍵 `Ctrl+[` ≡ Esc 作保險。
- **CJK 即時輸入是現成 client 最大缺口、也是自製 client 最大機會**（新增 IS-5 / AC-5）：Termius 的終端 raw 模式擋掉 IME 即時組字、中文只能他處組好再貼上。自製 Flutter 端可設計 compose buffer 讓 IME 先組字再送出、繞過 raw 模式——這是超越現成 client、而非追平的差異化點。
- **傳輸選擇帶來 CJK 顯示的先天優勢**（回饋進 Out of Scope 的 mosh 段）：app_tunnel 走 WebSocket、不做本地回顯預測，先天避開「mosh 預測撞 CJK 雙寬字導致中文輸入行錯位」——現成 client 的 mosh 路線開中文輸入時會錯位、只能退回純 SSH（失去漫遊）。
- **連線失敗的 UX 要分逾時 vs 被拒**（新增 AC-6）：Phase 1 踩到「手機不在 tailnet → 連線逾時、卻直覺去查伺服器 sshd」的誤判。逾時多半是可達性層（沒進 tailnet）、被拒才是服務層，app 的錯誤訊息應引導正確方向。
- **假設 1 獲正面佐證**（回饋進 Reality Test）：實測 zellij session 在斷線期間任務未中斷、重連 attach 接回，印證「session 復原由 remote multiplexer 承擔、client 只需重連正確」，IS-2 風險降一級。

實測細節記錄在外部工作流的實作記錄與手機 client 選型文（blog 專案）。這些回饋不改本提案「Phase 1 凍結範圍後才 promote」的路徑——它們讓 In Scope 候選更精確（特別是 IS-5 這個新機會），凍結時一併納入。
